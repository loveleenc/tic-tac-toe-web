...
const extractUser = async (request: LoggedInUserRequest, _response: Response, next: NextFunction) => {
    try {
        const jwtPayload = await userService.verifyTokenButIgnoreExpiry(request.token);
        if (jwtPayload === null) {
            throw new Error("Decoded token appears to be invalid");
        }
        else if (jwtPayload.exp as number <= Math.round((Date.now() + 60000)/ 1000)) {    //Additional minute added
            request.accessTokenExpired = true;
        }
        request.user = await userService.extractUserFromJwtPayload(jwtPayload);
        next();
    }
    catch (error: unknown) {
        next(error);
...
const refreshToken = async (request: LoggedInUserRequest, response: Response, next: NextFunction) => {
    try {
        if (request.accessTokenExpired) {
            if (request.refreshToken === undefined) {
                throw new Error("Refresh token is unavailable")
            }
            const userInRefreshToken = await userService.extractUserFromRefreshToken(request.refreshToken);

            if(userInRefreshToken?.username !== request.user.username){
                throw new errors.AuthenticationError();
            }
            else{
                const accessToken = userService.getNewAccessToken(request.user);
